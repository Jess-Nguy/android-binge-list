...
public class reviews extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    public static final String TAG = "==REVIEW ACTIVITY==";
    ...
    /**
     * Read the database table for personal reviews of a movie.
    ...
    public void getData() {
        ...
        String[] projection = {MyDbHelper.ID, MyDbHelper.MOVIETITLE, MyDbHelper.YEAR, MyDbHelper.IMAGE, MyDbHelper.COMMENT, MyDbHelper.RATING};
        // db.query will retrieve the data and return a Cursor to access it
        Cursor mycursor = db.query(MyDbHelper.MYREVIEWTABLE, projection, null,
                null, null, null, null);
        if (mycursor != null) {
            myRowItems = new ArrayList<RowItem>();
            myListView = findViewById(R.id.reviewListView);
            // Loop through our returned results from the start
            while (mycursor.moveToNext()) {
                String title = mycursor.getString(
                        1);
                String year = mycursor.getString(2);
                String poster = mycursor.getString(mycursor.getColumnIndex(MyDbHelper.IMAGE));
                String comment = mycursor.getString(mycursor.getColumnIndex(MyDbHelper.COMMENT));
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.browserMenuItem:
                ...
                Intent switchActivityBrowse = new Intent(reviews.this, MainActivity.class);
                startActivity(switchActivityBrowse);
                break;
            case R.id.favouriteMenuItem:
                // switch fragment
                Log.d(TAG, "Favourite Menu Item");
                Intent switchActivityFavourites = new Intent(reviews.this, favourites.class);
                startActivity(switchActivityFavourites);
                break;
            case R.id.profileMenuItem:
                // switch fragment
                Log.d(TAG, "Profile Menu Item");
                Intent switchActivityProfile = new Intent(reviews.this, profile.class);
